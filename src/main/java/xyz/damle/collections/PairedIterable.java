 ...
 *
 * <p>Given two {@link Iterable} instances {@code A} and {@code B}, the {@link #iterator} method on
 * this iterator provide a Pair of {@code A.next()} and {@code B.next()} until one of the iterators
 * is exhausted. This can simplify zipping over two iterables using the for-each construct. Example
 * usage:
 *
 * <pre>{@code
...
public class PairedIterable<L, R> implements Iterable<PairedItem<L, R>> {
  ...
  /**
   * Convenience static factory to construct the PairedIterable from provided {@link Iterable}
   * sources.
   *
   * @param leftIterable the iterable for the left side element.
   * @param rightIterable the iterable for the right side element.
   ...
   * @since 4.5
   */
  public static <L, R> PairedIterable<L, R> of(
      Iterable<L> leftIterable, Iterable<R> rightIterable) {
